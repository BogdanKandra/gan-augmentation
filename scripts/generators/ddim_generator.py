...
from scripts.models.ddpm import Unet
from scripts.models.ddim import DDIMModel

LOGGER = utils.get_logger(__name__)


class DDIM_Generator(AbstractGenerator):
    """ Class representing a generator for TorchVision datasets,
    using a Denoising Diffusion Implicit Model (DDIM) """
    def preprocess_dataset(self) -> None:
        ...
        if not self.preprocessed:
            self.hyperparams = copy(config.DDIM_GEN_HYPERPARAMS)
    ...
    def build_model(self, compute_batch_size: bool = False) -> None:
        ...
        self.model = DDIMModel(
            ...
            ddim_sampling_eta=self.hyperparams["SAMPLING_ETA"],
            ddim_sampling_steps=self.hyperparams["SAMPLING_STEPS"]
        )

        if compute_batch_size:
            if self.device.type == "cuda":
                LOGGER.info(">>> GPU detected, using a conservative batch size due to DDIM memory requirements.")
                # DDIM is memory intensive, so we use a smaller batch size
    ...
    def train_model(self,
        ...
        Note: DDIM uses the same training process as DDPM, but with a different sampling process during generation.
        ...
        with mlflow.start_run(run_id=self.run_id, log_system_metrics=True):
            ...
            with torch.no_grad():
                ...
                for X_batch, y_batch in tqdm(self.test_dataloader):
                    ...
                    # Generate samples with the same labels using DDIM sampling
