...
class CNN_Classifier(AbstractClassifier):
    ...
    def build_model(self, compute_batch_size: bool = False) -> None:
        ...
        """
        self.model = CNN(self.dataset_type).to(self.device, non_blocking=self.non_blocking)

        if compute_batch_size:
            if self.device.type == "cuda":
                LOGGER.info(">>> Searching for the optimal batch size for this model and GPU...")
                temp_model = CNN(self.dataset_type).to(self.device, non_blocking=self.non_blocking)
